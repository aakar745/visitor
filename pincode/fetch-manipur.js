/**
 * Fetch Manipur PIN codes specifically
 * Quick fetch for Manipur state only
 * 
 * Usage: node fetch-manipur.js
 */
...
class ManipurFetcher {
  constructor() {
    // ALL Manipur PIN code ranges (complete coverage)
    this.manipurRanges = [
      { start: 795001, end: 795999 },  // Covers all 16 districts of Manipur
    ];
    
    // Calculate total PINs to fetch
    this.totalPins = this.manipurRanges.reduce((sum, range) => 
  ...
  async fetch() {
    console.log(chalk.blue.bold('\n🚀 Fetching ALL Manipur PIN Codes (Complete)\n'));
    console.log(chalk.gray('━'.repeat(60)));
    console.log(chalk.cyan(`📍 Districts: All 16 districts of Manipur`));
    ...
    // Fetch all ranges
    for (const range of this.manipurRanges) {
  ...
  async saveData() {
    const filename = path.join('./data/raw', `manipur-${Date.now()}.json`);
  ...
  showSummary() {
    ...
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next step: node process-manipur.js\n'));
  }
}

// Run fetcher
const fetcher = new ManipurFetcher();
