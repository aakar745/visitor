/**
 * Process Odisha PIN codes for import
 * Extracts State, City, Pincode WITH Area field
 * 
 * Usage: node process-odisha.js
 */
...
class OdishaProcessor {
  ...
  async process() {
    console.log(chalk.blue.bold('\n🔄 Processing Odisha Data\n'));
    console.log(chalk.gray('━'.repeat(60)));

    // Load all Odisha raw data files
    const rawFiles = await this.loadRawData();
    
    if (rawFiles.length === 0) {
      console.log(chalk.red('\n❌ No Odisha data found!'));
      console.log(chalk.yellow('👉 Run: node fetch-odisha.js first\n'));
      return;
    }
  ...
  async loadRawData() {
    ...
    const odishaFiles = files.filter(f => f.startsWith('odisha-'));
    
    const allData = [];
    for (const file of odishaFiles) {
  ...
  async saveData() {
    ...
    // Save as JSON
    await fs.writeJson(
      path.join(this.processedDir, 'odisha-states.json'),
      Array.from(this.states.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'odisha-cities.json'),
      Array.from(this.cities.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'odisha-pincodes.json'),
  ...
  async exportToCSV() {
    ...
    await fs.writeFile(
      path.join(this.outputDir, 'odisha-bulk-import.csv'),
      csvContent,
      'utf8'
    );

    console.log(chalk.green(`✅ Exported: odisha-bulk-import.csv`));
  }
  ...
  showSummary() {
    ...
    console.log(chalk.white('   • data/processed/odisha-states.json'));
    console.log(chalk.white('   • data/processed/odisha-cities.json'));
    console.log(chalk.white('   • data/processed/odisha-pincodes.json'));
    console.log(chalk.cyan('\n📋 Ready for import:'));
    console.log(chalk.white('   • data/output/excel/odisha-bulk-import.csv'));
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next: Import odisha-bulk-import.csv via Admin Panel\n'));
    console.log(chalk.cyan('📝 Note: Area field included for each post office\n'));
  }
}

// Run processor
const processor = new OdishaProcessor();
