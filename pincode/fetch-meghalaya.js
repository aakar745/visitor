/**
 * Fetch Meghalaya PIN codes specifically
 * Quick fetch for Meghalaya state only
 * 
 * Usage: node fetch-meghalaya.js
 */
...
class MeghalayaFetcher {
  constructor() {
    // ALL Meghalaya PIN code ranges (complete coverage)
    this.meghalayaRanges = [
      { start: 793001, end: 793999 },  // East Khasi Hills (Shillong), West Khasi Hills
      { start: 794001, end: 794999 },  // Jaintia Hills, Garo Hills, Ri Bhoi
    ];
    
    // Calculate total PINs to fetch
    this.totalPins = this.meghalayaRanges.reduce((sum, range) => 
  ...
  async fetch() {
    console.log(chalk.blue.bold('\n🚀 Fetching ALL Meghalaya PIN Codes (Complete)\n'));
    console.log(chalk.gray('━'.repeat(60)));
    console.log(chalk.cyan(`📍 Districts: All 11 districts of Meghalaya`));
    ...
    // Fetch all ranges
    for (const range of this.meghalayaRanges) {
  ...
  async saveData() {
    const filename = path.join('./data/raw', `meghalaya-${Date.now()}.json`);
  ...
  showSummary() {
    ...
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next step: node process-meghalaya.js\n'));
  }
}

// Run fetcher
const fetcher = new MeghalayaFetcher();
