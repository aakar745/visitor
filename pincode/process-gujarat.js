/**
 * Process Gujarat PIN codes for import
 * Extracts State, City, Pincode WITHOUT Area field
 * 
 * Usage: node process-gujarat.js
 */
...
class GujaratProcessor {
  ...
  async process() {
    ...
    // Load all Gujarat raw data files
    const rawFiles = await this.loadRawData();
    
    if (rawFiles.length === 0) {
      console.log(chalk.red('\n❌ No Gujarat data found!'));
      console.log(chalk.yellow('👉 Run: node fetch-gujarat.js first\n'));
      return;
    }
  ...
  async loadRawData() {
    ...
    const gujaratFiles = files.filter(f => f.startsWith('gujarat-'));
    
    const allData = [];
    for (const file of gujaratFiles) {
  ...
  async saveData() {
    ...
    // Save as JSON
    await fs.writeJson(
      path.join(this.processedDir, 'gujarat-states.json'),
      Array.from(this.states.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'gujarat-cities.json'),
      Array.from(this.cities.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'gujarat-pincodes.json'),
  ...
  async exportToCSV() {
    ...
    await fs.writeFile(
      path.join(this.outputDir, 'gujarat-bulk-import.csv'),
      csvContent,
      'utf8'
    );

    console.log(chalk.green(`✅ Exported: gujarat-bulk-import.csv`));
  }
  ...
  showSummary() {
    ...
    console.log(chalk.white('   • data/processed/gujarat-states.json'));
    console.log(chalk.white('   • data/processed/gujarat-cities.json'));
    console.log(chalk.white('   • data/processed/gujarat-pincodes.json'));
    console.log(chalk.cyan('\n📋 Ready for import:'));
    console.log(chalk.white('   • data/output/excel/gujarat-bulk-import.csv'));
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next: Import gujarat-bulk-import.csv via Admin Panel\n'));
    console.log(chalk.cyan('📝 Note: Area field included for each post office\n'));
  }
}

// Run processor
const processor = new GujaratProcessor();
