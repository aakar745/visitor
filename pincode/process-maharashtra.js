/**
 * Process Maharashtra PIN codes for import
 * Extracts State, City, Pincode WITH Area field
 * 
 * Usage: node process-maharashtra.js
 */
...
class MaharashtraProcessor {
  ...
  async process() {
    console.log(chalk.blue.bold('\n🔄 Processing Maharashtra Data\n'));
    console.log(chalk.gray('━'.repeat(60)));

    // Load all Maharashtra raw data files
    const rawFiles = await this.loadRawData();
    
    if (rawFiles.length === 0) {
      console.log(chalk.red('\n❌ No Maharashtra data found!'));
      console.log(chalk.yellow('👉 Run: node fetch-maharashtra.js first\n'));
      return;
    }
  ...
  async loadRawData() {
    ...
    const maharashtraFiles = files.filter(f => f.startsWith('maharashtra-'));
    
    const allData = [];
    for (const file of maharashtraFiles) {
  ...
  async saveData() {
    ...
    // Save as JSON
    await fs.writeJson(
      path.join(this.processedDir, 'maharashtra-states.json'),
      Array.from(this.states.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'maharashtra-cities.json'),
      Array.from(this.cities.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'maharashtra-pincodes.json'),
  ...
  async exportToCSV() {
    ...
    await fs.writeFile(
      path.join(this.outputDir, 'maharashtra-bulk-import.csv'),
      csvContent,
      'utf8'
    );

    console.log(chalk.green(`✅ Exported: maharashtra-bulk-import.csv`));
  }
  ...
  showSummary() {
    ...
    console.log(chalk.white('   • data/processed/maharashtra-states.json'));
    console.log(chalk.white('   • data/processed/maharashtra-cities.json'));
    console.log(chalk.white('   • data/processed/maharashtra-pincodes.json'));
    console.log(chalk.cyan('\n📋 Ready for import:'));
    console.log(chalk.white('   • data/output/excel/maharashtra-bulk-import.csv'));
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next: Import maharashtra-bulk-import.csv via Admin Panel\n'));
    console.log(chalk.cyan('📝 Note: Area field included for each post office\n'));
  }
}

// Run processor
const processor = new MaharashtraProcessor();
