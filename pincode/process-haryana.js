/**
 * Process Haryana PIN codes for import
 * Extracts State, City, Pincode WITH Area field
 * 
 * Usage: node process-haryana.js
 */
...
class HaryanaProcessor {
  ...
  async process() {
    console.log(chalk.blue.bold('\n🔄 Processing Haryana Data\n'));
    console.log(chalk.gray('━'.repeat(60)));

    // Load all Haryana raw data files
    const rawFiles = await this.loadRawData();
    
    if (rawFiles.length === 0) {
      console.log(chalk.red('\n❌ No Haryana data found!'));
      console.log(chalk.yellow('👉 Run: node fetch-haryana.js first\n'));
      return;
    }
  ...
  async loadRawData() {
    ...
    const haryanaFiles = files.filter(f => f.startsWith('haryana-'));
    
    const allData = [];
    for (const file of haryanaFiles) {
  ...
  async saveData() {
    ...
    // Save as JSON
    await fs.writeJson(
      path.join(this.processedDir, 'haryana-states.json'),
      Array.from(this.states.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'haryana-cities.json'),
      Array.from(this.cities.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'haryana-pincodes.json'),
  ...
  async exportToCSV() {
    ...
    await fs.writeFile(
      path.join(this.outputDir, 'haryana-bulk-import.csv'),
      csvContent,
      'utf8'
    );

    console.log(chalk.green(`✅ Exported: haryana-bulk-import.csv`));
  }
  ...
  showSummary() {
    ...
    console.log(chalk.white('   • data/processed/haryana-states.json'));
    console.log(chalk.white('   • data/processed/haryana-cities.json'));
    console.log(chalk.white('   • data/processed/haryana-pincodes.json'));
    console.log(chalk.cyan('\n📋 Ready for import:'));
    console.log(chalk.white('   • data/output/excel/haryana-bulk-import.csv'));
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next: Import haryana-bulk-import.csv via Admin Panel\n'));
    console.log(chalk.cyan('📝 Note: Area field included for each post office\n'));
  }
}

// Run processor
const processor = new HaryanaProcessor();
