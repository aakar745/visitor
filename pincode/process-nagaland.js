/**
 * Process Nagaland PIN codes for import
 * Extracts State, City, Pincode WITH Area field
 * 
 * Usage: node process-nagaland.js
 */
...
class NagalandProcessor {
  ...
  async process() {
    console.log(chalk.blue.bold('\n🔄 Processing Nagaland Data\n'));
    console.log(chalk.gray('━'.repeat(60)));

    // Load all Nagaland raw data files
    const rawFiles = await this.loadRawData();
    
    if (rawFiles.length === 0) {
      console.log(chalk.red('\n❌ No Nagaland data found!'));
      console.log(chalk.yellow('👉 Run: node fetch-nagaland.js first\n'));
      return;
    }
  ...
  async loadRawData() {
    ...
    const nagalandFiles = files.filter(f => f.startsWith('nagaland-'));
    
    const allData = [];
    for (const file of nagalandFiles) {
  ...
  async saveData() {
    ...
    // Save as JSON
    await fs.writeJson(
      path.join(this.processedDir, 'nagaland-states.json'),
      Array.from(this.states.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'nagaland-cities.json'),
      Array.from(this.cities.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'nagaland-pincodes.json'),
  ...
  async exportToCSV() {
    ...
    await fs.writeFile(
      path.join(this.outputDir, 'nagaland-bulk-import.csv'),
      csvContent,
      'utf8'
    );

    console.log(chalk.green(`✅ Exported: nagaland-bulk-import.csv`));
  }
  ...
  showSummary() {
    ...
    console.log(chalk.white('   • data/processed/nagaland-states.json'));
    console.log(chalk.white('   • data/processed/nagaland-cities.json'));
    console.log(chalk.white('   • data/processed/nagaland-pincodes.json'));
    console.log(chalk.cyan('\n📋 Ready for import:'));
    console.log(chalk.white('   • data/output/excel/nagaland-bulk-import.csv'));
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next: Import nagaland-bulk-import.csv via Admin Panel\n'));
    console.log(chalk.cyan('📝 Note: Area field included for each post office\n'));
  }
}

// Run processor
const processor = new NagalandProcessor();
