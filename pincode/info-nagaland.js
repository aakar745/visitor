/**
 * Show Nagaland fetch information
 */

const chalk = require('chalk');

const nagalandRanges = [
  { start: 797001, end: 797999, name: 'All districts (Kohima, Dimapur, Mokokchung, Tuensang, etc.)' },
];

const totalPins = nagalandRanges.reduce((sum, range) => 
  sum + (range.end - range.start + 1), 0
);

console.log(chalk.blue.bold('\n📊 Nagaland Complete Fetch Information\n'));
console.log(chalk.gray('━'.repeat(75)));

console.log(chalk.cyan('\n📍 Coverage:'));
console.log(chalk.white(`   • Total ranges: ${nagalandRanges.length}`));
...
nagalandRanges.forEach((range, idx) => {
...
console.log(chalk.white('   node fetch-nagaland.js\n'));
console.log(chalk.yellow('💡 Tip: The script auto-saves progress every 500 PINs, safe to stop anytime!\n'));
console.log(chalk.cyan('📝 Note: Includes Kohima (capital), Dimapur and all 16 districts\n'));
