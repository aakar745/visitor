/**
 * Process Telangana PIN codes for import
 * Extracts State, City, Pincode WITH Area field
 * 
 * Usage: node process-telangana.js
 */
...
class TelanganaProcessor {
  ...
  async process() {
    console.log(chalk.blue.bold('\n🔄 Processing Telangana Data\n'));
    console.log(chalk.gray('━'.repeat(60)));

    // Load all Telangana raw data files
    const rawFiles = await this.loadRawData();
    
    if (rawFiles.length === 0) {
      console.log(chalk.red('\n❌ No Telangana data found!'));
      console.log(chalk.yellow('👉 Run: node fetch-telangana.js first\n'));
      return;
    }
  ...
  async loadRawData() {
    ...
    const telanganaFiles = files.filter(f => f.startsWith('telangana-'));
    
    const allData = [];
    for (const file of telanganaFiles) {
  ...
  async saveData() {
    ...
    // Save as JSON
    await fs.writeJson(
      path.join(this.processedDir, 'telangana-states.json'),
      Array.from(this.states.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'telangana-cities.json'),
      Array.from(this.cities.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'telangana-pincodes.json'),
  ...
  async exportToCSV() {
    ...
    await fs.writeFile(
      path.join(this.outputDir, 'telangana-bulk-import.csv'),
      csvContent,
      'utf8'
    );

    console.log(chalk.green(`✅ Exported: telangana-bulk-import.csv`));
  }
  ...
  showSummary() {
    ...
    console.log(chalk.white('   • data/processed/telangana-states.json'));
    console.log(chalk.white('   • data/processed/telangana-cities.json'));
    console.log(chalk.white('   • data/processed/telangana-pincodes.json'));
    console.log(chalk.cyan('\n📋 Ready for import:'));
    console.log(chalk.white('   • data/output/excel/telangana-bulk-import.csv'));
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next: Import telangana-bulk-import.csv via Admin Panel\n'));
    console.log(chalk.cyan('📝 Note: Area field included for each post office\n'));
  }
}

// Run processor
const processor = new TelanganaProcessor();
