/**
 * Process Sikkim PIN codes for import
 * Extracts State, City, Pincode WITH Area field
 * 
 * Usage: node process-sikkim.js
 */
...
class SikkimProcessor {
  ...
  async process() {
    console.log(chalk.blue.bold('\n🔄 Processing Sikkim Data\n'));
    console.log(chalk.gray('━'.repeat(60)));

    // Load all Sikkim raw data files
    const rawFiles = await this.loadRawData();
    
    if (rawFiles.length === 0) {
      console.log(chalk.red('\n❌ No Sikkim data found!'));
      console.log(chalk.yellow('👉 Run: node fetch-sikkim.js first\n'));
      return;
    }
  ...
  async loadRawData() {
    ...
    const sikkimFiles = files.filter(f => f.startsWith('sikkim-'));
    
    const allData = [];
    for (const file of sikkimFiles) {
  ...
  async saveData() {
    ...
    // Save as JSON
    await fs.writeJson(
      path.join(this.processedDir, 'sikkim-states.json'),
      Array.from(this.states.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'sikkim-cities.json'),
      Array.from(this.cities.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'sikkim-pincodes.json'),
  ...
  async exportToCSV() {
    ...
    await fs.writeFile(
      path.join(this.outputDir, 'sikkim-bulk-import.csv'),
      csvContent,
      'utf8'
    );

    console.log(chalk.green(`✅ Exported: sikkim-bulk-import.csv`));
  }
  ...
  showSummary() {
    ...
    console.log(chalk.white('   • data/processed/sikkim-states.json'));
    console.log(chalk.white('   • data/processed/sikkim-cities.json'));
    console.log(chalk.white('   • data/processed/sikkim-pincodes.json'));
    console.log(chalk.cyan('\n📋 Ready for import:'));
    console.log(chalk.white('   • data/output/excel/sikkim-bulk-import.csv'));
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next: Import sikkim-bulk-import.csv via Admin Panel\n'));
    console.log(chalk.cyan('📝 Note: Area field included for each post office\n'));
  }
}

// Run processor
const processor = new SikkimProcessor();
