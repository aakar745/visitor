/**
 * Process Karnataka PIN codes for import
 * Extracts State, City, Pincode WITH Area field
 * 
 * Usage: node process-karnataka.js
 */
...
class KarnatakaProcessor {
  ...
  async process() {
    ...
    // Load all Karnataka raw data files
    const rawFiles = await this.loadRawData();
    
    if (rawFiles.length === 0) {
      console.log(chalk.red('\n❌ No Karnataka data found!'));
      console.log(chalk.yellow('👉 Run: node fetch-karnataka.js first\n'));
      return;
    }
  ...
  async loadRawData() {
    ...
    const karnatakaFiles = files.filter(f => f.startsWith('karnataka-'));
    
    const allData = [];
    for (const file of karnatakaFiles) {
  ...
  async saveData() {
    ...
    // Save as JSON
    await fs.writeJson(
      path.join(this.processedDir, 'karnataka-states.json'),
      Array.from(this.states.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'karnataka-cities.json'),
      Array.from(this.cities.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'karnataka-pincodes.json'),
  ...
  async exportToCSV() {
    ...
    await fs.writeFile(
      path.join(this.outputDir, 'karnataka-bulk-import.csv'),
      csvContent,
      'utf8'
    );

    console.log(chalk.green(`✅ Exported: karnataka-bulk-import.csv`));
  }
  ...
  showSummary() {
    ...
    console.log(chalk.white('   • data/processed/karnataka-states.json'));
    console.log(chalk.white('   • data/processed/karnataka-cities.json'));
    console.log(chalk.white('   • data/processed/karnataka-pincodes.json'));
    console.log(chalk.cyan('\n📋 Ready for import:'));
    console.log(chalk.white('   • data/output/excel/karnataka-bulk-import.csv'));
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next: Import karnataka-bulk-import.csv via Admin Panel\n'));
    console.log(chalk.cyan('📝 Note: Area field included for each post office\n'));
  }
}

// Run processor
const processor = new KarnatakaProcessor();
