/**
 * Process Uttar Pradesh PIN codes for import
 * Extracts State, City, Pincode WITH Area field
 * 
 * Usage: node process-uttar-pradesh.js
 */
...
class UttarPradeshProcessor {
  ...
  async process() {
    console.log(chalk.blue.bold('\n🔄 Processing Uttar Pradesh Data\n'));
    console.log(chalk.gray('━'.repeat(60)));

    // Load all Uttar Pradesh raw data files
    const rawFiles = await this.loadRawData();
    
    if (rawFiles.length === 0) {
      console.log(chalk.red('\n❌ No Uttar Pradesh data found!'));
      console.log(chalk.yellow('👉 Run: node fetch-uttar-pradesh.js first\n'));
      return;
    }
  ...
  async loadRawData() {
    ...
    const uttarPradeshFiles = files.filter(f => f.startsWith('uttar-pradesh-'));
    
    const allData = [];
    for (const file of uttarPradeshFiles) {
  ...
  async saveData() {
    ...
    // Save as JSON
    await fs.writeJson(
      path.join(this.processedDir, 'uttar-pradesh-states.json'),
      Array.from(this.states.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'uttar-pradesh-cities.json'),
      Array.from(this.cities.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'uttar-pradesh-pincodes.json'),
  ...
  async exportToCSV() {
    ...
    await fs.writeFile(
      path.join(this.outputDir, 'uttar-pradesh-bulk-import.csv'),
      csvContent,
      'utf8'
    );

    console.log(chalk.green(`✅ Exported: uttar-pradesh-bulk-import.csv`));
  }
  ...
  showSummary() {
    ...
    console.log(chalk.white('   • data/processed/uttar-pradesh-states.json'));
    console.log(chalk.white('   • data/processed/uttar-pradesh-cities.json'));
    console.log(chalk.white('   • data/processed/uttar-pradesh-pincodes.json'));
    console.log(chalk.cyan('\n📋 Ready for import:'));
    console.log(chalk.white('   • data/output/excel/uttar-pradesh-bulk-import.csv'));
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next: Import uttar-pradesh-bulk-import.csv via Admin Panel\n'));
    console.log(chalk.cyan('📝 Note: Area field included for each post office\n'));
  }
}

// Run processor
const processor = new UttarPradeshProcessor();
