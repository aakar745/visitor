/**
 * Process Tripura PIN codes for import
 * Extracts State, City, Pincode WITH Area field
 * 
 * Usage: node process-tripura.js
 */
...
class TripuraProcessor {
  ...
  async process() {
    console.log(chalk.blue.bold('\n🔄 Processing Tripura Data\n'));
    console.log(chalk.gray('━'.repeat(60)));

    // Load all Tripura raw data files
    const rawFiles = await this.loadRawData();
    
    if (rawFiles.length === 0) {
      console.log(chalk.red('\n❌ No Tripura data found!'));
      console.log(chalk.yellow('👉 Run: node fetch-tripura.js first\n'));
      return;
    }
  ...
  async loadRawData() {
    ...
    const tripuraFiles = files.filter(f => f.startsWith('tripura-'));
    
    const allData = [];
    for (const file of tripuraFiles) {
  ...
  async saveData() {
    ...
    // Save as JSON
    await fs.writeJson(
      path.join(this.processedDir, 'tripura-states.json'),
      Array.from(this.states.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'tripura-cities.json'),
      Array.from(this.cities.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'tripura-pincodes.json'),
  ...
  async exportToCSV() {
    ...
    await fs.writeFile(
      path.join(this.outputDir, 'tripura-bulk-import.csv'),
      csvContent,
      'utf8'
    );

    console.log(chalk.green(`✅ Exported: tripura-bulk-import.csv`));
  }
  ...
  showSummary() {
    ...
    console.log(chalk.white('   • data/processed/tripura-states.json'));
    console.log(chalk.white('   • data/processed/tripura-cities.json'));
    console.log(chalk.white('   • data/processed/tripura-pincodes.json'));
    console.log(chalk.cyan('\n📋 Ready for import:'));
    console.log(chalk.white('   • data/output/excel/tripura-bulk-import.csv'));
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next: Import tripura-bulk-import.csv via Admin Panel\n'));
    console.log(chalk.cyan('📝 Note: Area field included for each post office\n'));
  }
}

// Run processor
const processor = new TripuraProcessor();
