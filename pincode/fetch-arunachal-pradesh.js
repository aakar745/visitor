/**
 * Fetch Arunachal Pradesh PIN codes specifically
 * Quick fetch for Arunachal Pradesh state only
 * 
 * Usage: node fetch-arunachal-pradesh.js
 */
...
class ArunachalPradeshFetcher {
  constructor() {
    // ALL Arunachal Pradesh PIN code ranges (complete coverage)
    this.arunachalPradeshRanges = [
      { start: 790001, end: 790999 },  // Papum Pare, Itanagar (Capital)
      { start: 791001, end: 791999 },  // Lower Subansiri, East Kameng
      { start: 792001, end: 792999 },  // West Kameng, Tawang
      { start: 793001, end: 793999 },  // West Siang, Upper Siang
      { start: 794001, end: 794999 },  // East Siang, Dibang Valley
      { start: 795001, end: 795999 },  // Lohit, Anjaw, Changlang
      { start: 796001, end: 796999 },  // Tirap, Longding
    ];
    
    // Calculate total PINs to fetch
    this.totalPins = this.arunachalPradeshRanges.reduce((sum, range) => 
  ...
  async fetch() {
    console.log(chalk.blue.bold('\n🚀 Fetching ALL Arunachal Pradesh PIN Codes (Complete)\n'));
    console.log(chalk.gray('━'.repeat(60)));
    console.log(chalk.cyan(`📍 Districts: All 25 districts of Arunachal Pradesh`));
    ...
    // Fetch all ranges
    for (const range of this.arunachalPradeshRanges) {
  ...
  async saveData() {
    const filename = path.join('./data/raw', `arunachal-pradesh-${Date.now()}.json`);
  ...
  showSummary() {
    ...
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next step: node process-arunachal-pradesh.js\n'));
  }
}

// Run fetcher
const fetcher = new ArunachalPradeshFetcher();
