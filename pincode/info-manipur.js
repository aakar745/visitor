/**
 * Show Manipur fetch information
 */

const chalk = require('chalk');

const manipurRanges = [
  { start: 795001, end: 795999, name: 'All districts (Imphal East, Imphal West, Thoubal, Bishnupur, etc.)' },
];

const totalPins = manipurRanges.reduce((sum, range) => 
  sum + (range.end - range.start + 1), 0
);

console.log(chalk.blue.bold('\n📊 Manipur Complete Fetch Information\n'));
console.log(chalk.gray('━'.repeat(75)));

console.log(chalk.cyan('\n📍 Coverage:'));
console.log(chalk.white(`   • Total ranges: ${manipurRanges.length}`));
...
manipurRanges.forEach((range, idx) => {
...
console.log(chalk.white('   node fetch-manipur.js\n'));
console.log(chalk.yellow('💡 Tip: The script auto-saves progress every 500 PINs, safe to stop anytime!\n'));
console.log(chalk.cyan('📝 Note: Includes Imphal (capital) and all 16 districts of the northeastern state\n'));
