/**
 * Fetch Goa PIN codes specifically
 * Quick fetch for Goa state only
 * 
 * Usage: node fetch-goa.js
 */
...
class GoaFetcher {
  constructor() {
    // ALL Goa PIN code ranges (complete coverage)
    this.goaRanges = [
      { start: 403001, end: 403999 },  // North & South Goa (Panaji, Margao, Vasco, Mapusa)
    ];
    
    // Calculate total PINs to fetch
    this.totalPins = this.goaRanges.reduce((sum, range) => 
  ...
  async fetch() {
    console.log(chalk.blue.bold('\n🚀 Fetching ALL Goa PIN Codes (Complete)\n'));
    console.log(chalk.gray('━'.repeat(60)));
    console.log(chalk.cyan(`📍 Districts: Both districts of Goa (North & South)`));
    ...
    // Fetch all ranges (deduplicate to avoid double fetching)
    const uniquePins = new Set();
    for (const range of this.goaRanges) {
  ...
  async saveData() {
    const filename = path.join('./data/raw', `goa-${Date.now()}.json`);
  ...
  showSummary() {
    ...
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next step: node process-goa.js\n'));
  }
}

// Run fetcher
const fetcher = new GoaFetcher();
