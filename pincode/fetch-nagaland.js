/**
 * Fetch Nagaland PIN codes specifically
 * Quick fetch for Nagaland state only
 * 
 * Usage: node fetch-nagaland.js
 */
...
class NagalandFetcher {
  constructor() {
    // ALL Nagaland PIN code ranges (complete coverage)
    this.nagalandRanges = [
      { start: 797001, end: 797999 },  // Covers all 16 districts of Nagaland
    ];
    
    // Calculate total PINs to fetch
    this.totalPins = this.nagalandRanges.reduce((sum, range) => 
  ...
  async fetch() {
    console.log(chalk.blue.bold('\n🚀 Fetching ALL Nagaland PIN Codes (Complete)\n'));
    console.log(chalk.gray('━'.repeat(60)));
    console.log(chalk.cyan(`📍 Districts: All 16 districts of Nagaland`));
    ...
    // Fetch all ranges
    for (const range of this.nagalandRanges) {
  ...
  async saveData() {
    const filename = path.join('./data/raw', `nagaland-${Date.now()}.json`);
  ...
  showSummary() {
    ...
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next step: node process-nagaland.js\n'));
  }
}

// Run fetcher
const fetcher = new NagalandFetcher();
