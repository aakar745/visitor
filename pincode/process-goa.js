/**
 * Process Goa PIN codes for import
 * Extracts State, City, Pincode WITH Area field
 * 
 * Usage: node process-goa.js
 */
...
class GoaProcessor {
  ...
  async process() {
    ...
    // Load all Goa raw data files
    const rawFiles = await this.loadRawData();
    
    if (rawFiles.length === 0) {
      console.log(chalk.red('\n❌ No Goa data found!'));
      console.log(chalk.yellow('👉 Run: node fetch-goa.js first\n'));
      return;
    }
  ...
  async loadRawData() {
    ...
    const goaFiles = files.filter(f => f.startsWith('goa-'));
    
    const allData = [];
    for (const file of goaFiles) {
  ...
  async saveData() {
    ...
    // Save as JSON
    await fs.writeJson(
      path.join(this.processedDir, 'goa-states.json'),
      Array.from(this.states.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'goa-cities.json'),
      Array.from(this.cities.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'goa-pincodes.json'),
  ...
  async exportToCSV() {
    ...
    await fs.writeFile(
      path.join(this.outputDir, 'goa-bulk-import.csv'),
      csvContent,
      'utf8'
    );

    console.log(chalk.green(`✅ Exported: goa-bulk-import.csv`));
  }
  ...
  showSummary() {
    ...
    console.log(chalk.white('   • data/processed/goa-states.json'));
    console.log(chalk.white('   • data/processed/goa-cities.json'));
    console.log(chalk.white('   • data/processed/goa-pincodes.json'));
    console.log(chalk.cyan('\n📋 Ready for import:'));
    console.log(chalk.white('   • data/output/excel/goa-bulk-import.csv'));
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next: Import goa-bulk-import.csv via Admin Panel\n'));
    console.log(chalk.cyan('📝 Note: Area field included for each post office\n'));
  }
}

// Run processor
const processor = new GoaProcessor();
