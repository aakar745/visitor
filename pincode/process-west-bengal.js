/**
 * Process West Bengal PIN codes for import
 * Extracts State, City, Pincode WITH Area field
 * 
 * Usage: node process-west-bengal.js
 */
...
class WestBengalProcessor {
  ...
  async process() {
    console.log(chalk.blue.bold('\n🔄 Processing West Bengal Data\n'));
    console.log(chalk.gray('━'.repeat(60)));

    // Load all West Bengal raw data files
    const rawFiles = await this.loadRawData();
    
    if (rawFiles.length === 0) {
      console.log(chalk.red('\n❌ No West Bengal data found!'));
      console.log(chalk.yellow('👉 Run: node fetch-west-bengal.js first\n'));
      return;
    }
  ...
  async loadRawData() {
    ...
    const westBengalFiles = files.filter(f => f.startsWith('west-bengal-'));
    
    const allData = [];
    for (const file of westBengalFiles) {
  ...
  async saveData() {
    ...
    // Save as JSON
    await fs.writeJson(
      path.join(this.processedDir, 'west-bengal-states.json'),
      Array.from(this.states.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'west-bengal-cities.json'),
      Array.from(this.cities.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'west-bengal-pincodes.json'),
  ...
  async exportToCSV() {
    ...
    await fs.writeFile(
      path.join(this.outputDir, 'west-bengal-bulk-import.csv'),
      csvContent,
      'utf8'
    );

    console.log(chalk.green(`✅ Exported: west-bengal-bulk-import.csv`));
  }
  ...
  showSummary() {
    ...
    console.log(chalk.white('   • data/processed/west-bengal-states.json'));
    console.log(chalk.white('   • data/processed/west-bengal-cities.json'));
    console.log(chalk.white('   • data/processed/west-bengal-pincodes.json'));
    console.log(chalk.cyan('\n📋 Ready for import:'));
    console.log(chalk.white('   • data/output/excel/west-bengal-bulk-import.csv'));
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next: Import west-bengal-bulk-import.csv via Admin Panel\n'));
    console.log(chalk.cyan('📝 Note: Area field included for each post office\n'));
  }
}

// Run processor
const processor = new WestBengalProcessor();
