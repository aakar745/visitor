/**
 * Process Kerala PIN codes for import
 * Extracts State, City, Pincode WITH Area field
 * 
 * Usage: node process-kerala.js
 */
...
class KeralaProcessor {
  ...
  async process() {
    console.log(chalk.blue.bold('\n🔄 Processing Kerala Data\n'));
    console.log(chalk.gray('━'.repeat(60)));

    // Load all Kerala raw data files
    const rawFiles = await this.loadRawData();
    
    if (rawFiles.length === 0) {
      console.log(chalk.red('\n❌ No Kerala data found!'));
      console.log(chalk.yellow('👉 Run: node fetch-kerala.js first\n'));
      return;
    }
  ...
  async loadRawData() {
    ...
    const keralaFiles = files.filter(f => f.startsWith('kerala-'));
    
    const allData = [];
    for (const file of keralaFiles) {
  ...
  async saveData() {
    ...
    // Save as JSON
    await fs.writeJson(
      path.join(this.processedDir, 'kerala-states.json'),
      Array.from(this.states.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'kerala-cities.json'),
      Array.from(this.cities.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'kerala-pincodes.json'),
  ...
  async exportToCSV() {
    ...
    await fs.writeFile(
      path.join(this.outputDir, 'kerala-bulk-import.csv'),
      csvContent,
      'utf8'
    );

    console.log(chalk.green(`✅ Exported: kerala-bulk-import.csv`));
  }
  ...
  showSummary() {
    ...
    console.log(chalk.white('   • data/processed/kerala-states.json'));
    console.log(chalk.white('   • data/processed/kerala-cities.json'));
    console.log(chalk.white('   • data/processed/kerala-pincodes.json'));
    console.log(chalk.cyan('\n📋 Ready for import:'));
    console.log(chalk.white('   • data/output/excel/kerala-bulk-import.csv'));
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next: Import kerala-bulk-import.csv via Admin Panel\n'));
    console.log(chalk.cyan('📝 Note: Area field included for each post office\n'));
  }
}

// Run processor
const processor = new KeralaProcessor();
