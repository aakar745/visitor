/**
 * Fetch Sikkim PIN codes specifically
 * Quick fetch for Sikkim state only
 * 
 * Usage: node fetch-sikkim.js
 */
...
class SikkimFetcher {
  constructor() {
    // ALL Sikkim PIN code ranges (complete coverage)
    this.sikkimRanges = [
      { start: 737001, end: 737999 },  // Covers all 6 districts of Sikkim
    ];
    
    // Calculate total PINs to fetch
    this.totalPins = this.sikkimRanges.reduce((sum, range) => 
  ...
  async fetch() {
    console.log(chalk.blue.bold('\n🚀 Fetching ALL Sikkim PIN Codes (Complete)\n'));
    console.log(chalk.gray('━'.repeat(60)));
    console.log(chalk.cyan(`📍 Districts: All 6 districts of Sikkim`));
    ...
    // Fetch all ranges
    for (const range of this.sikkimRanges) {
  ...
  async saveData() {
    const filename = path.join('./data/raw', `sikkim-${Date.now()}.json`);
  ...
  showSummary() {
    ...
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next step: node process-sikkim.js\n'));
  }
}

// Run fetcher
const fetcher = new SikkimFetcher();
