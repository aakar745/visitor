/**
 * Process Arunachal Pradesh PIN codes for import
 * Extracts State, City, Pincode WITH Area field
 * 
 * Usage: node process-arunachal-pradesh.js
 */
...
class ArunachalPradeshProcessor {
  constructor() {
  ...
  async process() {
    console.log(chalk.blue.bold('\n🔄 Processing Arunachal Pradesh Data\n'));
    console.log(chalk.gray('━'.repeat(60)));

    // Load all Arunachal Pradesh raw data files
    const rawFiles = await this.loadRawData();
    
    if (rawFiles.length === 0) {
      console.log(chalk.red('\n❌ No Arunachal Pradesh data found!'));
      console.log(chalk.yellow('👉 Run: node fetch-arunachal-pradesh.js first\n'));
      return;
    }
  ...
  async loadRawData() {
    ...
    const arunachalPradeshFiles = files.filter(f => f.startsWith('arunachal-pradesh-'));
    
    const allData = [];
    for (const file of arunachalPradeshFiles) {
  ...
  async saveData() {
    ...
    // Save as JSON
    await fs.writeJson(
      path.join(this.processedDir, 'arunachal-pradesh-states.json'),
      Array.from(this.states.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'arunachal-pradesh-cities.json'),
      Array.from(this.cities.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'arunachal-pradesh-pincodes.json'),
  ...
  async exportToCSV() {
    ...
    await fs.writeFile(
      path.join(this.outputDir, 'arunachal-pradesh-bulk-import.csv'),
      csvContent,
      'utf8'
    );

    console.log(chalk.green(`✅ Exported: arunachal-pradesh-bulk-import.csv`));
  }
  ...
  showSummary() {
    ...
    console.log(chalk.white('   • data/processed/arunachal-pradesh-states.json'));
    console.log(chalk.white('   • data/processed/arunachal-pradesh-cities.json'));
    console.log(chalk.white('   • data/processed/arunachal-pradesh-pincodes.json'));
    console.log(chalk.cyan('\n📋 Ready for import:'));
    console.log(chalk.white('   • data/output/excel/arunachal-pradesh-bulk-import.csv'));
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next: Import arunachal-pradesh-bulk-import.csv via Admin Panel\n'));
    console.log(chalk.cyan('📝 Note: Area field included for each post office\n'));
  }
}

// Run processor
const processor = new ArunachalPradeshProcessor();
