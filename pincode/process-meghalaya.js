/**
 * Process Meghalaya PIN codes for import
 * Extracts State, City, Pincode WITH Area field
 * 
 * Usage: node process-meghalaya.js
 */
...
class MeghalayaProcessor {
  ...
  async process() {
    console.log(chalk.blue.bold('\n🔄 Processing Meghalaya Data\n'));
    console.log(chalk.gray('━'.repeat(60)));

    // Load all Meghalaya raw data files
    const rawFiles = await this.loadRawData();
    
    if (rawFiles.length === 0) {
      console.log(chalk.red('\n❌ No Meghalaya data found!'));
      console.log(chalk.yellow('👉 Run: node fetch-meghalaya.js first\n'));
      return;
    }
  ...
  async loadRawData() {
    ...
    const meghalayaFiles = files.filter(f => f.startsWith('meghalaya-'));
    
    const allData = [];
    for (const file of meghalayaFiles) {
  ...
  async saveData() {
    ...
    // Save as JSON
    await fs.writeJson(
      path.join(this.processedDir, 'meghalaya-states.json'),
      Array.from(this.states.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'meghalaya-cities.json'),
      Array.from(this.cities.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'meghalaya-pincodes.json'),
  ...
  async exportToCSV() {
    ...
    await fs.writeFile(
      path.join(this.outputDir, 'meghalaya-bulk-import.csv'),
      csvContent,
      'utf8'
    );

    console.log(chalk.green(`✅ Exported: meghalaya-bulk-import.csv`));
  }
  ...
  showSummary() {
    ...
    console.log(chalk.white('   • data/processed/meghalaya-states.json'));
    console.log(chalk.white('   • data/processed/meghalaya-cities.json'));
    console.log(chalk.white('   • data/processed/meghalaya-pincodes.json'));
    console.log(chalk.cyan('\n📋 Ready for import:'));
    console.log(chalk.white('   • data/output/excel/meghalaya-bulk-import.csv'));
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next: Import meghalaya-bulk-import.csv via Admin Panel\n'));
    console.log(chalk.cyan('📝 Note: Area field included for each post office\n'));
  }
}

// Run processor
const processor = new MeghalayaProcessor();
