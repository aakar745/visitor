/**
 * Process Manipur PIN codes for import
 * Extracts State, City, Pincode WITH Area field
 * 
 * Usage: node process-manipur.js
 */
...
class ManipurProcessor {
  ...
  async process() {
    console.log(chalk.blue.bold('\n🔄 Processing Manipur Data\n'));
    console.log(chalk.gray('━'.repeat(60)));

    // Load all Manipur raw data files
    const rawFiles = await this.loadRawData();
    
    if (rawFiles.length === 0) {
      console.log(chalk.red('\n❌ No Manipur data found!'));
      console.log(chalk.yellow('👉 Run: node fetch-manipur.js first\n'));
      return;
    }
  ...
  async loadRawData() {
    ...
    const manipurFiles = files.filter(f => f.startsWith('manipur-'));
    
    const allData = [];
    for (const file of manipurFiles) {
  ...
  async saveData() {
    ...
    // Save as JSON
    await fs.writeJson(
      path.join(this.processedDir, 'manipur-states.json'),
      Array.from(this.states.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'manipur-cities.json'),
      Array.from(this.cities.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'manipur-pincodes.json'),
  ...
  async exportToCSV() {
    ...
    await fs.writeFile(
      path.join(this.outputDir, 'manipur-bulk-import.csv'),
      csvContent,
      'utf8'
    );

    console.log(chalk.green(`✅ Exported: manipur-bulk-import.csv`));
  }
  ...
  showSummary() {
    ...
    console.log(chalk.white('   • data/processed/manipur-states.json'));
    console.log(chalk.white('   • data/processed/manipur-cities.json'));
    console.log(chalk.white('   • data/processed/manipur-pincodes.json'));
    console.log(chalk.cyan('\n📋 Ready for import:'));
    console.log(chalk.white('   • data/output/excel/manipur-bulk-import.csv'));
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next: Import manipur-bulk-import.csv via Admin Panel\n'));
    console.log(chalk.cyan('📝 Note: Area field included for each post office\n'));
  }
}

// Run processor
const processor = new ManipurProcessor();
