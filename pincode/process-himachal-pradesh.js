/**
 * Process Himachal Pradesh PIN codes for import
 * Extracts State, City, Pincode WITH Area field
 * 
 * Usage: node process-himachal-pradesh.js
 */
...
class HimachalPradeshProcessor {
  constructor() {
  ...
  async process() {
    ...
    // Load all Himachal Pradesh raw data files
    const rawFiles = await this.loadRawData();
    
    if (rawFiles.length === 0) {
      console.log(chalk.red('\n❌ No Himachal Pradesh data found!'));
      console.log(chalk.yellow('👉 Run: node fetch-himachal-pradesh.js first\n'));
      return;
    }
  ...
  async loadRawData() {
    ...
    const himachalPradeshFiles = files.filter(f => f.startsWith('himachal-pradesh-'));
    
    const allData = [];
    for (const file of himachalPradeshFiles) {
  ...
  async saveData() {
    ...
    // Save as JSON
    await fs.writeJson(
      path.join(this.processedDir, 'himachal-pradesh-states.json'),
      Array.from(this.states.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'himachal-pradesh-cities.json'),
      Array.from(this.cities.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'himachal-pradesh-pincodes.json'),
  ...
  async exportToCSV() {
    ...
    await fs.writeFile(
      path.join(this.outputDir, 'himachal-pradesh-bulk-import.csv'),
      csvContent,
      'utf8'
    );

    console.log(chalk.green(`✅ Exported: himachal-pradesh-bulk-import.csv`));
  }
  ...
  showSummary() {
    ...
    console.log(chalk.white('   • data/processed/himachal-pradesh-states.json'));
    console.log(chalk.white('   • data/processed/himachal-pradesh-cities.json'));
    console.log(chalk.white('   • data/processed/himachal-pradesh-pincodes.json'));
    console.log(chalk.cyan('\n📋 Ready for import:'));
    console.log(chalk.white('   • data/output/excel/himachal-pradesh-bulk-import.csv'));
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next: Import himachal-pradesh-bulk-import.csv via Admin Panel\n'));
    console.log(chalk.cyan('📝 Note: Area field included for each post office\n'));
  }
}

// Run processor
const processor = new HimachalPradeshProcessor();
