/**
 * Process Madhya Pradesh PIN codes for import
 * Extracts State, City, Pincode WITH Area field
 * 
 * Usage: node process-madhya-pradesh.js
 */
...
class MadhyaPradeshProcessor {
  ...
  async process() {
    ...
    // Load all Madhya Pradesh raw data files
    const rawFiles = await this.loadRawData();
    
    if (rawFiles.length === 0) {
      console.log(chalk.red('\n❌ No Madhya Pradesh data found!'));
      console.log(chalk.yellow('👉 Run: node fetch-madhya-pradesh.js first\n'));
      return;
    }
  ...
  async loadRawData() {
    ...
    const madhyaPradeshFiles = files.filter(f => f.startsWith('madhya-pradesh-'));
    
    const allData = [];
    for (const file of madhyaPradeshFiles) {
  ...
  async saveData() {
    ...
    // Save as JSON
    await fs.writeJson(
      path.join(this.processedDir, 'madhya-pradesh-states.json'),
      Array.from(this.states.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'madhya-pradesh-cities.json'),
      Array.from(this.cities.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'madhya-pradesh-pincodes.json'),
  ...
  async exportToCSV() {
    ...
    await fs.writeFile(
      path.join(this.outputDir, 'madhya-pradesh-bulk-import.csv'),
      csvContent,
      'utf8'
    );

    console.log(chalk.green(`✅ Exported: madhya-pradesh-bulk-import.csv`));
  }
  ...
  showSummary() {
    ...
    console.log(chalk.white('   • data/processed/madhya-pradesh-states.json'));
    console.log(chalk.white('   • data/processed/madhya-pradesh-cities.json'));
    console.log(chalk.white('   • data/processed/madhya-pradesh-pincodes.json'));
    console.log(chalk.cyan('\n📋 Ready for import:'));
    console.log(chalk.white('   • data/output/excel/madhya-pradesh-bulk-import.csv'));
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next: Import madhya-pradesh-bulk-import.csv via Admin Panel\n'));
    console.log(chalk.cyan('📝 Note: Area field included for each post office\n'));
  }
}

// Run processor
const processor = new MadhyaPradeshProcessor();
