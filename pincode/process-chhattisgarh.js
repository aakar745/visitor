/**
 * Process Chhattisgarh PIN codes for import
 * Extracts State, City, Pincode WITH Area field
 * 
 * Usage: node process-chhattisgarh.js
 */
...
class ChhattisgarhProcessor {
  ...
  async process() {
    console.log(chalk.blue.bold('\n🔄 Processing Chhattisgarh Data\n'));
    console.log(chalk.gray('━'.repeat(60)));

    // Load all Chhattisgarh raw data files
    const rawFiles = await this.loadRawData();
    
    if (rawFiles.length === 0) {
      console.log(chalk.red('\n❌ No Chhattisgarh data found!'));
      console.log(chalk.yellow('👉 Run: node fetch-chhattisgarh.js first\n'));
      return;
    }
  ...
  async loadRawData() {
    ...
    const chhattisgarhFiles = files.filter(f => f.startsWith('chhattisgarh-'));
    
    const allData = [];
    for (const file of chhattisgarhFiles) {
  ...
  async saveData() {
    ...
    // Save as JSON
    await fs.writeJson(
      path.join(this.processedDir, 'chhattisgarh-states.json'),
      Array.from(this.states.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'chhattisgarh-cities.json'),
      Array.from(this.cities.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'chhattisgarh-pincodes.json'),
  ...
  async exportToCSV() {
    ...
    await fs.writeFile(
      path.join(this.outputDir, 'chhattisgarh-bulk-import.csv'),
      csvContent,
      'utf8'
    );

    console.log(chalk.green(`✅ Exported: chhattisgarh-bulk-import.csv`));
  }
  ...
  showSummary() {
    ...
    console.log(chalk.white('   • data/processed/chhattisgarh-states.json'));
    console.log(chalk.white('   • data/processed/chhattisgarh-cities.json'));
    console.log(chalk.white('   • data/processed/chhattisgarh-pincodes.json'));
    console.log(chalk.cyan('\n📋 Ready for import:'));
    console.log(chalk.white('   • data/output/excel/chhattisgarh-bulk-import.csv'));
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next: Import chhattisgarh-bulk-import.csv via Admin Panel\n'));
    console.log(chalk.cyan('📝 Note: Area field included for each post office\n'));
  }
}

// Run processor
const processor = new ChhattisgarhProcessor();
