/**
 * Fetch Assam PIN codes specifically
 * Quick fetch for Assam state only
 * 
 * Usage: node fetch-assam.js
 */
...
class AssamFetcher {
  constructor() {
    // ALL Assam PIN code ranges (complete coverage)
    this.assamRanges = [
      { start: 781001, end: 781999 },  // Kamrup, Guwahati
      { start: 782001, end: 782999 },  // Nagaon, Morigaon
      { start: 783001, end: 783999 },  // Bongaigaon, Kokrajhar, Dhubri
      { start: 784001, end: 784999 },  // Lakhimpur, Dhemaji
      { start: 785001, end: 785999 },  // Jorhat, Sivasagar, Golaghat
      { start: 786001, end: 786999 },  // Dibrugarh, Tinsukia
      { start: 787001, end: 787999 },  // Karbi Anglong, North Cachar Hills
      { start: 788001, end: 788999 },  // Cachar, Karimganj, Hailakandi
    ];
    
    // Calculate total PINs to fetch
    this.totalPins = this.assamRanges.reduce((sum, range) => 
  ...
  async fetch() {
    console.log(chalk.blue.bold('\n🚀 Fetching ALL Assam PIN Codes (Complete)\n'));
    console.log(chalk.gray('━'.repeat(60)));
    console.log(chalk.cyan(`📍 Districts: All 33 districts of Assam`));
    ...
    // Fetch all ranges
    for (const range of this.assamRanges) {
  ...
  async saveData() {
    const filename = path.join('./data/raw', `assam-${Date.now()}.json`);
  ...
  showSummary() {
    ...
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next step: node process-assam.js\n'));
  }
}

// Run fetcher
const fetcher = new AssamFetcher();
