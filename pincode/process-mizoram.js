/**
 * Process Mizoram PIN codes for import
 * Extracts State, City, Pincode WITH Area field
 * 
 * Usage: node process-mizoram.js
 */
...
class MizoramProcessor {
  ...
  async process() {
    console.log(chalk.blue.bold('\n🔄 Processing Mizoram Data\n'));
    console.log(chalk.gray('━'.repeat(60)));

    // Load all Mizoram raw data files
    const rawFiles = await this.loadRawData();
    
    if (rawFiles.length === 0) {
      console.log(chalk.red('\n❌ No Mizoram data found!'));
      console.log(chalk.yellow('👉 Run: node fetch-mizoram.js first\n'));
      return;
    }
  ...
  async loadRawData() {
    ...
    const mizoramFiles = files.filter(f => f.startsWith('mizoram-'));
    
    const allData = [];
    for (const file of mizoramFiles) {
  ...
  async saveData() {
    ...
    // Save as JSON
    await fs.writeJson(
      path.join(this.processedDir, 'mizoram-states.json'),
      Array.from(this.states.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'mizoram-cities.json'),
      Array.from(this.cities.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'mizoram-pincodes.json'),
  ...
  async exportToCSV() {
    ...
    await fs.writeFile(
      path.join(this.outputDir, 'mizoram-bulk-import.csv'),
      csvContent,
      'utf8'
    );

    console.log(chalk.green(`✅ Exported: mizoram-bulk-import.csv`));
  }
  ...
  showSummary() {
    ...
    console.log(chalk.white('   • data/processed/mizoram-states.json'));
    console.log(chalk.white('   • data/processed/mizoram-cities.json'));
    console.log(chalk.white('   • data/processed/mizoram-pincodes.json'));
    console.log(chalk.cyan('\n📋 Ready for import:'));
    console.log(chalk.white('   • data/output/excel/mizoram-bulk-import.csv'));
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next: Import mizoram-bulk-import.csv via Admin Panel\n'));
    console.log(chalk.cyan('📝 Note: Area field included for each post office\n'));
  }
}

// Run processor
const processor = new MizoramProcessor();
