/**
 * Fetch Chhattisgarh PIN codes specifically
 * Quick fetch for Chhattisgarh state only
 * 
 * Usage: node fetch-chhattisgarh.js
 */
...
class ChhattisgarhFetcher {
  constructor() {
    // ALL Chhattisgarh PIN code ranges (complete coverage)
    this.chhattisgarhRanges = [
      { start: 490001, end: 490999 },  // Durg, Bhilai
      { start: 491001, end: 491999 },  // Durg Rural, Rajnandgaon
      { start: 492001, end: 492999 },  // Raipur
      { start: 493001, end: 493999 },  // Raipur Rural, Mahasamund
      { start: 494001, end: 494999 },  // Bastar, Jagdalpur
      { start: 495001, end: 495999 },  // Bilaspur, Janjgir-Champa
      { start: 496001, end: 496999 },  // Raigarh, Jashpur
      { start: 497001, end: 497999 },  // Surguja, Korba
    ];
    
    // Calculate total PINs to fetch
    this.totalPins = this.chhattisgarhRanges.reduce((sum, range) => 
  ...
  async fetch() {
    console.log(chalk.blue.bold('\n🚀 Fetching ALL Chhattisgarh PIN Codes (Complete)\n'));
    console.log(chalk.gray('━'.repeat(60)));
    console.log(chalk.cyan(`📍 Districts: All 27 districts of Chhattisgarh`));
    ...
    // Fetch all ranges
    for (const range of this.chhattisgarhRanges) {
  ...
  async saveData() {
    const filename = path.join('./data/raw', `chhattisgarh-${Date.now()}.json`);
  ...
  showSummary() {
    ...
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next step: node process-chhattisgarh.js\n'));
  }
}

// Run fetcher
const fetcher = new ChhattisgarhFetcher();
