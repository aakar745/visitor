/**
 * Process Bihar PIN codes for import
 * Extracts State, City, Pincode WITH Area field
 * 
 * Usage: node process-bihar.js
 */
...
class BiharProcessor {
  ...
  async process() {
    console.log(chalk.blue.bold('\n🔄 Processing Bihar Data\n'));
    console.log(chalk.gray('━'.repeat(60)));

    // Load all Bihar raw data files
    const rawFiles = await this.loadRawData();
    
    if (rawFiles.length === 0) {
      console.log(chalk.red('\n❌ No Bihar data found!'));
      console.log(chalk.yellow('👉 Run: node fetch-bihar.js first\n'));
      return;
    }
  ...
  async loadRawData() {
    ...
    const biharFiles = files.filter(f => f.startsWith('bihar-'));
    
    const allData = [];
    for (const file of biharFiles) {
  ...
  async saveData() {
    ...
    // Save as JSON
    await fs.writeJson(
      path.join(this.processedDir, 'bihar-states.json'),
      Array.from(this.states.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'bihar-cities.json'),
      Array.from(this.cities.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'bihar-pincodes.json'),
  ...
  async exportToCSV() {
    ...
    await fs.writeFile(
      path.join(this.outputDir, 'bihar-bulk-import.csv'),
      csvContent,
      'utf8'
    );

    console.log(chalk.green(`✅ Exported: bihar-bulk-import.csv`));
  }
  ...
  showSummary() {
    ...
    console.log(chalk.white('   • data/processed/bihar-states.json'));
    console.log(chalk.white('   • data/processed/bihar-cities.json'));
    console.log(chalk.white('   • data/processed/bihar-pincodes.json'));
    console.log(chalk.cyan('\n📋 Ready for import:'));
    console.log(chalk.white('   • data/output/excel/bihar-bulk-import.csv'));
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next: Import bihar-bulk-import.csv via Admin Panel\n'));
    console.log(chalk.cyan('📝 Note: Area field included for each post office\n'));
  }
}

// Run processor
const processor = new BiharProcessor();
