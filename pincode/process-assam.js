/**
 * Process Assam PIN codes for import
 * Extracts State, City, Pincode WITH Area field
 * 
 * Usage: node process-assam.js
 */
...
class AssamProcessor {
  ...
  async process() {
    console.log(chalk.blue.bold('\n🔄 Processing Assam Data\n'));
    console.log(chalk.gray('━'.repeat(60)));

    // Load all Assam raw data files
    const rawFiles = await this.loadRawData();
    
    if (rawFiles.length === 0) {
      console.log(chalk.red('\n❌ No Assam data found!'));
      console.log(chalk.yellow('👉 Run: node fetch-assam.js first\n'));
      return;
    }
  ...
  async loadRawData() {
    ...
    const assamFiles = files.filter(f => f.startsWith('assam-'));
    
    const allData = [];
    for (const file of assamFiles) {
  ...
  async saveData() {
    ...
    // Save as JSON
    await fs.writeJson(
      path.join(this.processedDir, 'assam-states.json'),
      Array.from(this.states.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'assam-cities.json'),
      Array.from(this.cities.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'assam-pincodes.json'),
  ...
  async exportToCSV() {
    ...
    await fs.writeFile(
      path.join(this.outputDir, 'assam-bulk-import.csv'),
      csvContent,
      'utf8'
    );

    console.log(chalk.green(`✅ Exported: assam-bulk-import.csv`));
  }
  ...
  showSummary() {
    ...
    console.log(chalk.white('   • data/processed/assam-states.json'));
    console.log(chalk.white('   • data/processed/assam-cities.json'));
    console.log(chalk.white('   • data/processed/assam-pincodes.json'));
    console.log(chalk.cyan('\n📋 Ready for import:'));
    console.log(chalk.white('   • data/output/excel/assam-bulk-import.csv'));
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next: Import assam-bulk-import.csv via Admin Panel\n'));
    console.log(chalk.cyan('📝 Note: Area field included for each post office\n'));
  }
}

// Run processor
const processor = new AssamProcessor();
