/**
 * Process Tamil Nadu PIN codes for import
 * Extracts State, City, Pincode WITH Area field
 * 
 * Usage: node process-tamil-nadu.js
 */
...
class TamilNaduProcessor {
  ...
  async process() {
    console.log(chalk.blue.bold('\n🔄 Processing Tamil Nadu Data\n'));
    console.log(chalk.gray('━'.repeat(60)));

    // Load all Tamil Nadu raw data files
    const rawFiles = await this.loadRawData();
    
    if (rawFiles.length === 0) {
      console.log(chalk.red('\n❌ No Tamil Nadu data found!'));
      console.log(chalk.yellow('👉 Run: node fetch-tamil-nadu.js first\n'));
      return;
    }
  ...
  async loadRawData() {
    ...
    const tamilNaduFiles = files.filter(f => f.startsWith('tamil-nadu-'));
    
    const allData = [];
    for (const file of tamilNaduFiles) {
  ...
  async saveData() {
    ...
    // Save as JSON
    await fs.writeJson(
      path.join(this.processedDir, 'tamil-nadu-states.json'),
      Array.from(this.states.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'tamil-nadu-cities.json'),
      Array.from(this.cities.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'tamil-nadu-pincodes.json'),
  ...
  async exportToCSV() {
    ...
    await fs.writeFile(
      path.join(this.outputDir, 'tamil-nadu-bulk-import.csv'),
      csvContent,
      'utf8'
    );

    console.log(chalk.green(`✅ Exported: tamil-nadu-bulk-import.csv`));
  }
  ...
  showSummary() {
    ...
    console.log(chalk.white('   • data/processed/tamil-nadu-states.json'));
    console.log(chalk.white('   • data/processed/tamil-nadu-cities.json'));
    console.log(chalk.white('   • data/processed/tamil-nadu-pincodes.json'));
    console.log(chalk.cyan('\n📋 Ready for import:'));
    console.log(chalk.white('   • data/output/excel/tamil-nadu-bulk-import.csv'));
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next: Import tamil-nadu-bulk-import.csv via Admin Panel\n'));
    console.log(chalk.cyan('📝 Note: Area field included for each post office\n'));
  }
}

// Run processor
const processor = new TamilNaduProcessor();
