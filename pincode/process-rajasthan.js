/**
 * Process Rajasthan PIN codes for import
 * Extracts State, City, Pincode WITH Area field
 * 
 * Usage: node process-rajasthan.js
 */
...
class RajasthanProcessor {
  ...
  async process() {
    console.log(chalk.blue.bold('\n🔄 Processing Rajasthan Data\n'));
    console.log(chalk.gray('━'.repeat(60)));

    // Load all Rajasthan raw data files
    const rawFiles = await this.loadRawData();
    
    if (rawFiles.length === 0) {
      console.log(chalk.red('\n❌ No Rajasthan data found!'));
      console.log(chalk.yellow('👉 Run: node fetch-rajasthan.js first\n'));
      return;
    }
  ...
  async loadRawData() {
    ...
    const rajasthanFiles = files.filter(f => f.startsWith('rajasthan-'));
    
    const allData = [];
    for (const file of rajasthanFiles) {
  ...
  async saveData() {
    ...
    // Save as JSON
    await fs.writeJson(
      path.join(this.processedDir, 'rajasthan-states.json'),
      Array.from(this.states.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'rajasthan-cities.json'),
      Array.from(this.cities.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'rajasthan-pincodes.json'),
  ...
  async exportToCSV() {
    ...
    await fs.writeFile(
      path.join(this.outputDir, 'rajasthan-bulk-import.csv'),
      csvContent,
      'utf8'
    );

    console.log(chalk.green(`✅ Exported: rajasthan-bulk-import.csv`));
  }
  ...
  showSummary() {
    ...
    console.log(chalk.white('   • data/processed/rajasthan-states.json'));
    console.log(chalk.white('   • data/processed/rajasthan-cities.json'));
    console.log(chalk.white('   • data/processed/rajasthan-pincodes.json'));
    console.log(chalk.cyan('\n📋 Ready for import:'));
    console.log(chalk.white('   • data/output/excel/rajasthan-bulk-import.csv'));
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next: Import rajasthan-bulk-import.csv via Admin Panel\n'));
    console.log(chalk.cyan('📝 Note: Area field included for each post office\n'));
  }
}

// Run processor
const processor = new RajasthanProcessor();
