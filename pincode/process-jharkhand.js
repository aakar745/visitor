/**
 * Process Jharkhand PIN codes for import
 * Extracts State, City, Pincode WITH Area field
 * 
 * Usage: node process-jharkhand.js
 */
...
class JharkhandProcessor {
  ...
  async process() {
    console.log(chalk.blue.bold('\n🔄 Processing Jharkhand Data\n'));
    console.log(chalk.gray('━'.repeat(60)));

    // Load all Jharkhand raw data files
    const rawFiles = await this.loadRawData();
    
    if (rawFiles.length === 0) {
      console.log(chalk.red('\n❌ No Jharkhand data found!'));
      console.log(chalk.yellow('👉 Run: node fetch-jharkhand.js first\n'));
      return;
    }
  ...
  async loadRawData() {
    ...
    const jharkhandFiles = files.filter(f => f.startsWith('jharkhand-'));
    
    const allData = [];
    for (const file of jharkhandFiles) {
  ...
  async saveData() {
    ...
    // Save as JSON
    await fs.writeJson(
      path.join(this.processedDir, 'jharkhand-states.json'),
      Array.from(this.states.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'jharkhand-cities.json'),
      Array.from(this.cities.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'jharkhand-pincodes.json'),
  ...
  async exportToCSV() {
    ...
    await fs.writeFile(
      path.join(this.outputDir, 'jharkhand-bulk-import.csv'),
      csvContent,
      'utf8'
    );

    console.log(chalk.green(`✅ Exported: jharkhand-bulk-import.csv`));
  }
  ...
  showSummary() {
    ...
    console.log(chalk.white('   • data/processed/jharkhand-states.json'));
    console.log(chalk.white('   • data/processed/jharkhand-cities.json'));
    console.log(chalk.white('   • data/processed/jharkhand-pincodes.json'));
    console.log(chalk.cyan('\n📋 Ready for import:'));
    console.log(chalk.white('   • data/output/excel/jharkhand-bulk-import.csv'));
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next: Import jharkhand-bulk-import.csv via Admin Panel\n'));
    console.log(chalk.cyan('📝 Note: Area field included for each post office\n'));
  }
}

// Run processor
const processor = new JharkhandProcessor();
