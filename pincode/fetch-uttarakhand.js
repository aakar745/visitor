/**
 * Fetch Uttarakhand PIN codes specifically
 * Quick fetch for Uttarakhand state only
 * 
 * Usage: node fetch-uttarakhand.js
 */
...
class UttarakhandFetcher {
  constructor() {
    // ALL Uttarakhand PIN code ranges (complete coverage)
    this.uttarakhandRanges = [
      { start: 244001, end: 244999 },  // Udham Singh Nagar (shares with UP)
      { start: 246001, end: 246999 },  // Haridwar, Dehradun (shares with UP)
      { start: 247001, end: 247999 },  // Dehradun, Haridwar
      { start: 248001, end: 248999 },  // Dehradun (Capital), Tehri Garhwal
      { start: 249001, end: 249999 },  // Haridwar, Dehradun Rural
      { start: 261001, end: 261999 },  // Udham Singh Nagar (shares with UP)
      { start: 262001, end: 262999 },  // Udham Singh Nagar, Nainital
      { start: 263001, end: 263999 },  // Nainital, Almora
      { start: 249401, end: 249410 },  // Rishikesh area (specific range)
    ];
    
    // Calculate total PINs to fetch
    this.totalPins = this.uttarakhandRanges.reduce((sum, range) => 
  ...
  async fetch() {
    console.log(chalk.blue.bold('\n🚀 Fetching ALL Uttarakhand PIN Codes (Complete)\n'));
    console.log(chalk.gray('━'.repeat(60)));
    console.log(chalk.cyan(`📍 Districts: All 13 districts of Uttarakhand`));
    ...
    // Fetch all ranges
    for (const range of this.uttarakhandRanges) {
  ...
  async saveData() {
    const filename = path.join('./data/raw', `uttarakhand-${Date.now()}.json`);
  ...
  showSummary() {
    ...
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next step: node process-uttarakhand.js\n'));
  }
}

// Run fetcher
const fetcher = new UttarakhandFetcher();
