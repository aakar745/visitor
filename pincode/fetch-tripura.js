/**
 * Fetch Tripura PIN codes specifically
 * Quick fetch for Tripura state only
 * 
 * Usage: node fetch-tripura.js
 */
...
class TripuraFetcher {
  constructor() {
    // ALL Tripura PIN code ranges (complete coverage)
    this.tripuraRanges = [
      { start: 799001, end: 799999 },  // Covers all 8 districts of Tripura
    ];
    
    // Calculate total PINs to fetch
    this.totalPins = this.tripuraRanges.reduce((sum, range) => 
  ...
  async fetch() {
    console.log(chalk.blue.bold('\n🚀 Fetching ALL Tripura PIN Codes (Complete)\n'));
    console.log(chalk.gray('━'.repeat(60)));
    console.log(chalk.cyan(`📍 Districts: All 8 districts of Tripura`));
    ...
    // Fetch all ranges
    for (const range of this.tripuraRanges) {
  ...
  async saveData() {
    const filename = path.join('./data/raw', `tripura-${Date.now()}.json`);
  ...
  showSummary() {
    ...
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next step: node process-tripura.js\n'));
  }
}

// Run fetcher
const fetcher = new TripuraFetcher();
