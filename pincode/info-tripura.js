/**
 * Show Tripura fetch information
 */

const chalk = require('chalk');

const tripuraRanges = [
  { start: 799001, end: 799999, name: 'All districts (Agartala, Ambassa, Belonia, Dharmanagar, Kailashahar, etc.)' },
];

const totalPins = tripuraRanges.reduce((sum, range) => 
  sum + (range.end - range.start + 1), 0
);

console.log(chalk.blue.bold('\n📊 Tripura Complete Fetch Information\n'));
console.log(chalk.gray('━'.repeat(75)));

console.log(chalk.cyan('\n📍 Coverage:'));
console.log(chalk.white(`   • Total ranges: ${tripuraRanges.length}`));
...
tripuraRanges.forEach((range, idx) => {
...
console.log(chalk.white('   node fetch-tripura.js\n'));
console.log(chalk.yellow('💡 Tip: The script auto-saves progress every 500 PINs, safe to stop anytime!\n'));
console.log(chalk.cyan('📝 Note: Includes Agartala (capital) and all 8 districts of the northeastern state\n'));
