/**
 * Process Andhra Pradesh PIN codes for import
 * Extracts State, City, Pincode WITH Area field
 * 
 * Usage: node process-andhra-pradesh.js
 */
...
class AndhraPradeshProcessor {
  ...
  async process() {
    console.log(chalk.blue.bold('\n🔄 Processing Andhra Pradesh Data\n'));
    console.log(chalk.gray('━'.repeat(60)));

    // Load all Andhra Pradesh raw data files
    const rawFiles = await this.loadRawData();
    
    if (rawFiles.length === 0) {
      console.log(chalk.red('\n❌ No Andhra Pradesh data found!'));
      console.log(chalk.yellow('👉 Run: node fetch-andhra-pradesh.js first\n'));
      return;
    }
  ...
  async loadRawData() {
    ...
    const andhraPradeshFiles = files.filter(f => f.startsWith('andhra-pradesh-'));
    
    const allData = [];
    for (const file of andhraPradeshFiles) {
  ...
  async saveData() {
    ...
    // Save as JSON
    await fs.writeJson(
      path.join(this.processedDir, 'andhra-pradesh-states.json'),
      Array.from(this.states.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'andhra-pradesh-cities.json'),
      Array.from(this.cities.values()),
      { spaces: 2 }
    );

    await fs.writeJson(
      path.join(this.processedDir, 'andhra-pradesh-pincodes.json'),
  ...
  async exportToCSV() {
    ...
    await fs.writeFile(
      path.join(this.outputDir, 'andhra-pradesh-bulk-import.csv'),
      csvContent,
      'utf8'
    );

    console.log(chalk.green(`✅ Exported: andhra-pradesh-bulk-import.csv`));
  }
  ...
  showSummary() {
    ...
    console.log(chalk.white('   • data/processed/andhra-pradesh-states.json'));
    console.log(chalk.white('   • data/processed/andhra-pradesh-cities.json'));
    console.log(chalk.white('   • data/processed/andhra-pradesh-pincodes.json'));
    console.log(chalk.cyan('\n📋 Ready for import:'));
    console.log(chalk.white('   • data/output/excel/andhra-pradesh-bulk-import.csv'));
    console.log(chalk.gray('\n' + '━'.repeat(60)));
    console.log(chalk.green('\n✨ Next: Import andhra-pradesh-bulk-import.csv via Admin Panel\n'));
    console.log(chalk.cyan('📝 Note: Area field included for each post office\n'));
  }
}

// Run processor
const processor = new AndhraPradeshProcessor();
