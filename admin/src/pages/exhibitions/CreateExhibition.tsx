...
const CreateExhibition: React.FC = () => {
  ...
  // Initialize with empty custom fields - users can add fields via FormBuilder
  React.useEffect(() => {
    // Set default fields only on mount if none exist
  ...
  // Handle exhibition name change to auto-generate slug
  ...
  const onFinish = async (values: any) => {
    // Prevent submission if not on last step or if canSubmit is false
    ...
    try {
      ...
      const exhibitionData: ExhibitionRequest = {
        name: values.name,
        tagline: values.tagline,
        description: values.description,
        venue: values.venue,
        registrationStartDate: toBackendDate(values.registrationStartDate, false),
        registrationEndDate: toBackendDate(values.registrationEndDate, false),
        onsiteStartDate: toBackendDate(values.onsiteStartDate, false),
        onsiteEndDate: toBackendDate(values.onsiteEndDate, false),
        ...
        paidEndDate: values.isPaid && values.paidEndDate ? toBackendDate(values.paidEndDate, false) : undefined,
      ...
      const errorMessage = error?.response?.data?.message || error?.message || 'Failed to create exhibition';
          ...
          <Row gutter={[24, 16]}>
            <Col xs={24}>
              ...
              <div style={{ 
                ...
                <Text style={{ fontSize: '13px' }}>
                  <strong>Timeline Flow:</strong> Registration Opens → Visitors Register → Registration Closes = Exhibition Ends
                </Text>
              </div>
            </Col>
              ...
              >
                <DatePicker
                  ...
                  disabledDate={(current) => {
                    ...
                    // Optionally: Exhibition should start after or on registration start
                    const regStartDate = form.getFieldValue('registrationStartDate');
                    if (regStartDate && current.isBefore(regStartDate.startOf('day'))) {
                      return true;
                    }
            ...
            <Col xs={24} md={12}>
              <Form.Item
                ...
                tooltip="Auto-filled from Registration Closes date"
                rules={[
                  { required: true, message: 'Exhibition end date is required' },
                  ({ getFieldValue }) => ({
                    validator(_, value) {
                      if (!value || !getFieldValue('onsiteStartDate')) {
                        return Promise.resolve();
                      }
                      if (value.isBefore(getFieldValue('onsiteStartDate'))) {
                        return Promise.reject(new Error('End date must be after start date'));
                      }
                      return Promise.resolve();
                    },
              ...
              >
                <DatePicker
                  placeholder="Auto-filled from registration close"
                  style={{ width: '100%' }}
                  size="middle"
                  showTime
                  format="DD/MM/YYYY HH:mm"
                  disabledDate={(current) => {
                    ...
                    // Disable dates before exhibition start date
                    const exhStartDate = form.getFieldValue('onsiteStartDate');
                    if (exhStartDate && current.isBefore(exhStartDate.startOf('day'))) {
                      return true;
                    }
    ...
    <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          ...
          <Text type="secondary">
            Set up a comprehensive exhibition with registration, pricing, and visitor management
          </Text>
        </div>
        <Space>
          <Button 
      ...
      >
        ...
        >
          <Space size="large">
            {currentStep > 0 && (
              <Button size="middle" onClick={prevStep} htmlType="button">
                Previous
              </Button>
            )}
            {currentStep < steps.length - 1 ? (
              <Button type="primary" size="middle" onClick={nextStep} htmlType="button">
                Next Step
              </Button>
            ) : (
              <Button 
